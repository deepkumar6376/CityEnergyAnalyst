...
def substation_main(locator, total_demand, building_names, settings, Flag):
    """
    ...
    T_DHS_supply = np.where(T_DHS > 0, T_DHS + settings.dT_heat, T_DHS)
    T_DCS_supply = np.where(Tcs != 1E6, Tcs - settings.dT_cool, 0)
    ...
    if Flag:
        ...
        for name in building_names:
            ...
            substation_model(locator, buildings[index], T_DHS, T_DHS_supply, T_DCS_supply, Ths, Tww, settings)
            index += 1
    else:
        index =0
        # calculate substation parameters per building
...
def substation_model(locator, building, t_DH, t_DH_supply, t_DC_supply, t_HS, t_WW, settings):
    '''
    ...
    if Qnom > 0:
        ...
        t_DH_return_hs, mcp_DH_hs, A_hex_hs = \
            calc_substation_heating(Qhsf, thi, tco, tci, cc, cc_0, Qnom, thi_0, tci_0, tco_0, settings)
    else:
    ...
    if Qnom > 0:
        ...
        t_DH_return_ww, mcp_DH_ww, A_hex_ww = \
            calc_substation_heating(Qwwf, thi, tco, tci, cc, cc_0, Qnom, thi_0, tci_0, tco_0, settings)
    else:
    ...
    if Qnom > 0:
        ...
        t_DC_return_cs, mcp_DC_cs, A_hex_cs = \
            calc_substation_cooling(Qcf, thi, tho, tci, ch, ch_0, Qnom, thi_0, tci_0, tho_0, settings)
    else:
    ...
    mdot_DH_result_flat = mcp_DH * 1000 / settings.cp  # convert from kW/K to kg/s
    mdot_heating_result_flat = mcp_DH_hs * 1000 / settings.cp  # convert from kW/K to kg/s
    mdot_dhw_result_flat = mcp_DH_ww * 1000 / settings.cp  # convert from kW/K to kg/s
    mdot_cool_result_flat = mcp_DC_cs * 1000 / settings.cp  # convert from kW/K to kg/s
...
def calc_substation_cooling(Q, thi, tho, tci, ch, ch_0, Qnom, thi_0, tci_0, tho_0, settings):
    '''
    ...
    Area_HEX_cooling, UA_cooling = calc_area_HEX(Qnom, dTm_0, settings.U_cool)
...
def calc_substation_heating(Q, thi, tco, tci, cc, cc_0, Qnom, thi_0, tci_0, tco_0, settings):
    '''
    ...
    Area_HEX_heating, UA_heating = calc_area_HEX(Qnom, dTm_0, settings.U_heat)
